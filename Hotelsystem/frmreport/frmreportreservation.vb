...
Public Class frmreportreservation
    ...
    Dim country As New tbcountry
    Dim cry As New CryReportCustNational
    ...
    Private Sub Frmreport_Sale_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            country.combocountry(cbcountry)
    ...
    Private Sub btncry_Click(sender As Object, e As EventArgs) Handles btncry.Click
        Try
            ...
            If cbcountry.SelectedValue = 0 Then
                subname = "%"
            Else
            ...
            dt.Clear()
            cn.connect()
            cm = New SqlCommand("select * from viewreserve where datereserve>='" & Format(CDate(date1.Text), "MM/dd/yyyy") & "'and datereserve<='" & Format(CDate(date2.Text), "MM/dd/yyyy") & "' and  countryname like N'%" & subname & "%' and statusid=2  ORDER BY reserveNO ASC", cn.conn)
            With da
                .SelectCommand = cm
                .Fill(dt)
            End With
