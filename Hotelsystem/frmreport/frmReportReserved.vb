...
Public Class frmReportReserved
    ...
    Private Sub btncry_Click(sender As Object, e As EventArgs) Handles btncry.Click
        Try
            ...
            dt.Clear()
            cn.connect()
            cm = New SqlCommand("select * from viewreserve where datereserve>='" & Format(CDate(date1.Text), "MM/dd/yyyy") & "'and datereserve<='" & Format(CDate(date2.Text), "MM/dd/yyyy") & "' and  bookingtypename like N'%" & subname & "%' and statusid=2  ORDER BY reserveNO ASC", cn.conn)
            With da
                .SelectCommand = cm
                .Fill(dt)
            End With
